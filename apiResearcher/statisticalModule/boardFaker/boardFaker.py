...
study = requests.get("http://localhost:3001/studies/_id=" + studyId)
study = study.json()
...
while(currentDocuments < numberOfDocuments):
    ...
    board = requests.get(f"http://localhost:3001/fetchBoard/_id={studyId}")
    board = board.json()
    boardId = board['_id']
    ...
    url = 'http://localhost:3001/boards/_id=' + boardId
